...
function successMessageRegister($message, $location) {
  ?>
  <script>
    setTimeout(function() {
      swal({
        title: 'Verify account in your Gmail',
        text: '<?php echo $message; ?>',
        type: 'success',
        timer: 3500,
        showCancelButton: false,
...
function customer($conn, $username, $password) {
  $email = $_POST['username'];
    ...
    if ($result->num_rows == 1) {
        ...
        NotVerified("CHECK EMAIL to Verify Email Account!");
...
function staff($conn, $username, $password) {
  $verify_status = '1';
  $sql = "SELECT * FROM `staff` WHERE email = ? AND verify_status = ? ";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("ss", $username, $verify_status );
  $stmt->execute();
  $result = $stmt->get_result();
...
function registerCustomer($conn, $username, $password){
    ...
    $mail->isSMTP();
      ...
      $address = $house_no . ' ' . $barangay . ', ' . $municipality . ', ' . $province . ', ' . $zip_code;
      ...
      if ($add_cust_stmt->execute() && $add_cust_profile->execute() && $add_cust_contacts->execute()) {
          ...
          successMessageRegister("Check Email for Verification", "../../index.php#about");    
      } else {
