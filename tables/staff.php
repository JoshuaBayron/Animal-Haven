    ...
    <tbody>
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>";
                echo "<td>
                        <a class='custom-button' href='edit-staff.php?staff_id=" . $row["staff_id"] . "&role=update-staff' style='background-color: #8B4000'>
                            Edit
                        </a>
                        <a class='custom-button' href='backend/crude.php?staff_id=" . $row["staff_id"] . "&role=delete-staff' onclick='return confirm(\"Are you sure you want to delete this staff?\")' style='background-color: red'>
                            Delete
                        </a>
                    </td>";
