                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) {
                      ...
                      if (stripos($payment, 'Not Yet Paid') !== false) {
                        ...
                        echo '<tr>
                                <td>
                                    <a class="custom-button" href="login-form/update-status.php?appointment_no=' . $row["appointment_id"] . '&animalName='.$row["animal_name"].'&customerEmail='.$row["customer_email"].'&staffEmail='.$row["staff_email"].'">Edit</a>
                                    <a class="custom-button" href="backend/crude.php?appointment_no=' . $row["appointment_id"] . '&role=archivingAppointment"  onclick="return confirm(\'Are you sure you want to delete this appointment?\')">Archive</a>
                                </td>
