...
if (isset($_GET['animals_id'])) {
    ...
    $sql_select = "SELECT * FROM animals WHERE animals_id = $pet_id";
    ...
    if ($result_select->num_rows > 0) {
        ...
        $sql_insert = "INSERT INTO animals_archive (animal_name, breed, species,gender,birthdate,age,quantity,customer_id) VALUES ('" . $pet_data['animal_name'] . "','" . $pet_data['breed'] . "','" . $pet_data['species'] . "','" . $pet_data['gender'] . "','" . $pet_data['birthdate'] . "','" . $pet_data['age'] . "','" . $pet_data['quantity'] . "','" . $pet_data['customer_id'] . "')";
        $conn->query($sql_insert);

        // Delete the pet from the pets table
        $sql_delete = "DELETE FROM animals WHERE animals_id  = $pet_id";
