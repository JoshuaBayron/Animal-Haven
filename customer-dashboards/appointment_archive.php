          ...
          <table id="outputTable" class="form-group" style="width: 100%; font-size: 12px;">
            <thead>
              <tr>
                ...
                <th style="width: 10%;background-color: #f34e4e;">Animals/Pets</th>
                <th style="width: 10%;background-color: #f34e4e;">Gender</th>
                <th style="width: 15%;background-color: #f34e4e;">Service</th>
                <th style="width: 15%;background-color: #f34e4e;">Start</th>
                <th style="width: 15%;background-color: #f34e4e;">Status</th>
            ...
            $sql = "SELECT * FROM appointment_archive AS app
            ...
            $animal_result = mysqli_query($conn, $sql);
                 ...
                 if (mysqli_num_rows($animal_result) > 0) {
                      ...
                      echo "<td>" . $counter_for_animals . "</td>";
                        ...
                        echo "<td>
                                ...
                                <a title='Restore Appointment' class='custom-button2'  data-id='" . $row["appointment_id"] . "' href='#' style=''><i class='fas fa-undo'></i> Restore</a><br>

                                <a title='Delete Appointment' class='custom-button3'  data-id='" . $row["appointment_id"] . "' href='#' style=''><i class='fas fa-trash-alt'></i> Permanent Delete</a>
             ...
             <button onclick="window.location.href='animal-status.php'" style="color:#E1D9D1;"><i class="fas fa-arrow-left" style="color: #E1D9D1;"></i></button>
            ...
            function confirmDelete(appointment_id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = `backend/restore_appointment.php?appointment_id=${appointment_id}`;
                    }
                });
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function confirmDelete(appointment_id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // If confirmed, redirect to delete.php with the record ID
                        window.location.href = `backend/delete_status.php?appointment_id=${appointment_id}`;
                    }
                });
            }
        });
