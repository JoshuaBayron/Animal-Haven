...
$pdf = new FPDF();
if (!$pdf) {
    die('FPDF instantiation failed.');
}

function getQueryBasedOnInterval($interval) {
    $commonColumns = "`appointment_id`, `appointment_service`, `appointment_status`, `start_event_date`, `end_event_date`, `payment_status`, `remarks`";
    
    switch ($interval) {
        case 'daily':
            return "SELECT $commonColumns FROM `appointment` WHERE DATE(`start_event_date`) = CURDATE() AND `appointment_service` IS NOT NULL AND `appointment_status` IS NOT NULL";
        case 'weekly':
            return "SELECT $commonColumns FROM `appointment` WHERE YEARWEEK(`start_event_date`, 1) = YEARWEEK(NOW(), 1) AND `appointment_service` IS NOT NULL AND `appointment_status` IS NOT NULL";
        case 'monthly':
            return "SELECT $commonColumns FROM `appointment` WHERE YEAR(`start_event_date`) = YEAR(NOW()) AND MONTH(`start_event_date`) = MONTH(NOW()) AND `appointment_service` IS NOT NULL AND `appointment_status` IS NOT NULL";
        default:
            return "SELECT $commonColumns FROM `appointment` WHERE `appointment_service` IS NOT NULL AND `appointment_status` IS NOT NULL";
    }
}
...
if (isset($_GET["printpdf"])) {
    ...
    switch ($interval) {
        case 'daily':
            $intervalTitle = 'Services Records For Online - ' . date('F j, Y');
            $serviceTitle = 'Services Rendered For the Day - ' . date('F j, Y');
            break;
        case 'weekly':
            $intervalTitle = 'Services Records For Online - Week of ' . date('F j, Y', strtotime('last Monday'));
            $serviceTitle = 'Services Rendered For Week of ' . date('F j, Y', strtotime('last Monday'));
            break;
        case 'monthly':
            $intervalTitle = 'Services Records For Online - ' . date('F Y');
            $serviceTitle = 'Services Rendered For the Month of ' . date('F Y');
            break;
        default:
            $intervalTitle = 'Services Records For Online';
    ...
    $pdf->Cell(30, 6, 'Payment Status', 1);
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell(30, 6, $row['payment_status'], 1);
        $pdf->Cell(30, 6, $row['remarks'], 1);
